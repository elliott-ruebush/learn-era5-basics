...
def _(mo):
    ...
    Since these are geospatial data, we should plot them on a real map!

    We use **Cartopy** (the standard Python mapping library) which integrates with matplotlib.

    ### Concepts: Map Layers
    Think of digital maps like a sandwich of layers:
    1.  **Base Layer**: The "canvas". Land, structural boundaries (coastlines), and context (borders).
    2.  **Data Layer**: Your actual values (Temperature) painted on top.
...
def _(ds, plt):
    ...
    for _ax, _city in zip(axes_base, _cities):
        # Zoom into the city's general area approximately
        # (For this demo we rely on Xarray's logical zoom, but here we just show features)
        ...
        # Set extent specifically for this view so we don't see the whole world
        # Approximate bounds for Chicago/NYC regions to match our data
        if _city == "Chicago":
